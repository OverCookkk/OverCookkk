...
wait()函数可依次拆分为三个操作：释放互斥锁、等待在条件变量上、再次获取互斥锁，

即**wait函数都在会阻塞时，自动释放锁权限，相当于调用unique_lock的成员函数unlock()，以便其他线程能有机会获得锁**。这就是条件变量只能和unique_lock一起使用的原因，否则当前线程一直占有锁，线程被阻塞。
